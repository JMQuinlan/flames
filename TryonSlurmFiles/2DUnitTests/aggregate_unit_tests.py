...
"""
===============================================================================
2D UNIT TEST AGGREGATOR
===============================================================================
Run after Run2DUnitTests.sh.  For every 2D unit test it:
...
Usage:
    ...
    python TryonSlurmFiles/2DUnitTests/aggregate_unit_tests.py --location desktop
    python TryonSlurmFiles/2DUnitTests/aggregate_unit_tests.py --only Couette_Single Laplace
===============================================================================
"""
...
_REPO = os.path.dirname(os.path.dirname(_HERE))          # TryonSlurmFiles/2DUnitTests -> repo
...
TESTS = [
    ...
    dict(name="Poiseuille", refdir="tests/FlowPoseuille/refrence",
         ...
         args=["{base}/FlowPoseuille/UNIT_TEST_2D/FlowPoiseuillex"]),
    # ---- Viscosity: Taylor-Green, Lamb-Oseen ----
    dict(name="TaylorGreen", refdir="tests/FlowTaylorGreenVortex/reference",
         script="taylor_green_single.py",
         args=["{base}/FlowTaylorGreenVortex/UNIT_TEST_2D/TaylorGreenDecay"]),
    dict(name="LambOseen", refdir="tests/FlowLambOseenVortex/reference",
         script="lamb_oseen_single.py",
         args=["{base}/FlowLambOseenVortex/UNIT_TEST_2D/FlowLambOseenVortex"]),
    ...
    dict(name="VortexShed_Re40", refdir="tests/FlowVortexShed/reference",
         ...
         args=["{base}/FlowVortexShed/UNIT_TEST_2D/output_hydro2"]),
...
def run_one(test, base, only):
    ...
    if outdir_arg and not outdir_arg.endswith("UNIT_TEST_2D") and not os.path.isdir(outdir_arg):
...
def main():
    ...
    print("=" * 70)
    print(f"2D UNIT TEST AGGREGATOR   location={a.location}   base={base}")
