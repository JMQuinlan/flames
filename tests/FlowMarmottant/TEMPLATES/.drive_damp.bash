#!/bin/bash
# 10-radius Marmottant sweep -- NSCBC with CORRECTED DAMPING (sigma 0.15 -> 0.6).
# Full domain, eta-only refinement, 10 ms, no time limit.  4 x 7 ranks.
set -u
GEN=/home/ttryon/Desktop/flames2/tests/FlowMarmottant/GENERATED_damp
BIN=/home/ttryon/Desktop/flames2/bin
OUT=$BIN/tests/FlowMarmottant/damp
NP=7
NPAR=4
mkdir -p $OUT/logs
for f in $GEN/input_R*; do
  ...
  grep -q "^nscbc.xlo.sigma = 0.6" "$f" || { echo "FATAL: damping not 0.6 in $f"; exit 1; }
...
echo "### preflight OK: 10/10 NSCBC sigma=0.6, eta-only, 10 ms, full domain"
cd $BIN
