...
B = "/home/ttryon/Desktop/flames2/bin/tests/FlowMarmottant/microbubble"
OUT = os.environ.get("FIG_OUT", os.path.dirname(os.path.abspath(__file__)))
SIGW, CHI, RB = 0.073, 0.55, 2.0e-6
...
for d in sorted(glob.glob(B + "/mb_*")):
    if not os.path.isdir(d):
        continue
    R0 = float(os.path.basename(d).split("_")[1]) * 1e-9
    s = series(d)
    if s:
        runs[R0] = s
        print(f"  loaded R0={R0*1e6:.3f} um  n={len(s)}  t_end={s[-1]['t']*1e9:.1f} ns")
...
print(f"  sigma_w={SIGW} N/m  chi={CHI} N/m  R_buck={RB*1e6:.3f} um  "
      f"R_rupt={RR*1e6:.4f} um  (elastic band {100*(RR/RB-1):.2f}% wide)")
print("=" * 100)
...
for R0 in sorted(runs):
    ...
    print(f"  {R0*1e6:8.3f} {R0/RB:6.3f} {reg:>9} {se:10.6f} | "
          f"{r[-1]['sig']:10.6f} {r[-1]['spr']:8.3f} {r[-1]['gam']:9.6f} "
          f"{100*(r[-1]['R']/r[0]['R']-1):9.4f} {max(x['u'] for x in r):10.5f} "
          f"{r[-1]['t']*1e9:9.1f}")
print("  " + "-" * 96)
full = sum(1 for R0 in runs if runs[R0][-1]['t'] > 0.95e-6)
...
cb.set_label(r"$R_0$  ($\mu$m)", fontsize=10.5)
...
for ext, dpi in (("pdf", None), ("png", 400)):
    p = os.path.join(OUT, f"fig_microbubble.{ext}")
