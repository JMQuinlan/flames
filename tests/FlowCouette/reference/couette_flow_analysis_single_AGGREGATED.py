"""
===============================================================================
COUETTE FLOW RIEMANN SOLVER COMPARISON SCRIPT
===============================================================================

PURPOSE:
    Compare numerical solutions from multiple Riemann solvers against the
    analytical solution for Couette flow. Generates comprehensive comparison
    plots and error analysis to evaluate solver performance.

REQUIRED INPUTS:
    Physical parameters and file paths in configuration section

DIRECTORY NAMING CONVENTION:
    The script expects output directories in the following format:
    
    CouetteFlow_Single_{RiemannSolver}
    
    Examples:
        CouetteFlow_Single_HLLC
        CouetteFlow_Single_Roe
        CouetteFlow_Single_HLLE
        CouetteFlow_Single_HLLC-Weno5
...
CONFIGURATION VARIABLES (lines 50-80):
    
    Physical Parameters:
        gamma                   - Ratio of specific heats (default: 1.4)
        pressure                - Reference pressure in Pa (default: 1.0e5)
        p0_tammann              - Tammann EOS pressure modification in Pa (default: 0.0)
        density                 - Fluid density in kg/m³ (default: 1.0)
        tube_height             - Height of the Couette flow domain in m (default: 0.01)
    ...
    File Paths:
        base_output_dir         - Path to directory containing CouetteFlow_Single_* folders

ANALYTICAL SOLUTION:
    Couette flow with stationary bottom plate and moving top plate:
        u(y) = U_top * (y / H)
    where:
        u(y) = velocity at height y
        U_top = top plate velocity
        H = channel height
...
OUTPUTS:
    ...
    1. Individual Solver Comparisons (per Riemann solver):
       - couette_{solver}_comparison.png/eps
       - Shows analytical solution vs. numerical solution for that solver
       - couette_{solver}_error_semilog.png/eps
       - Semi-log error plot for that solver
    
    2. Aggregated Comparison:
       - couette_aggregated_comparison.png/eps
       - All solvers compared against analytical solution
       - couette_aggregated_error_semilog.png/eps
       - Semi-log error plot for all solvers
...
USAGE:
    1. Set physical parameters and file paths in configuration section
    2. Ensure output directories follow naming convention
    3. Run script: python CouetteFlowRiemannComparison.py
...
def parse_output_directory(dir_name):
    ...
    Expected format: CouetteFlow_Single_{RiemannSolver}
    """
    prefix = 'CouetteFlow_Single_'
...
def calculate_errors(numerical_velocity, numerical_y, analytical_velocity, analytical_y):
...
print("=" * 80)
print("COUETTE FLOW RIEMANN SOLVER COMPARISON ANALYSIS")
print("=" * 80)
print(f"\nPhysical Parameters:")
print(f"  Gamma:              {gamma}")
...
for item in os.listdir(base_output_dir):
    ...
    if os.path.isdir(item_path) and item.startswith('CouetteFlow_Single_'):
...
if not all_data:
    ...
    print(f"Expected pattern: CouetteFlow_Single_{{RiemannSolver}}")
    exit(1)
...
for solver in riemann_solvers:
    ...
    title = f'Couette Flow - {format_label(solver)}'
    ...
    plt.tight_layout()
    output_filename = f'./Images/couette_{solver}_comparison'
    ...
    title = f'Couette Flow - {format_label(solver)} - Error Analysis'
    ax.set_title(title, fontsize=TITLE_FONTSIZE, fontweight='bold')
    ...
    plt.tight_layout()
    output_filename = f'./Images/couette_{solver}_error_semilog'
    plt.savefig(output_filename + '.png', format='png', dpi=300, bbox_inches='tight')
    plt.savefig(output_filename + '.eps', format='eps', bbox_inches='tight')
    print(f"  Saved: {output_filename}")
    plt.close()
...
title = 'Couette Flow - Riemann Solver Comparison'
if sim_time is not None:
    title += f' at t={sim_time:.6e}s'
ax.set_title(title, fontsize=TITLE_FONTSIZE, fontweight='bold')
...
plt.tight_layout()
output_filename = './Images/couette_aggregated_comparison'
plt.savefig(output_filename + '.png', format='png', dpi=300, bbox_inches='tight')
plt.savefig(output_filename + '.eps', format='eps', bbox_inches='tight')
print(f"Saved: {output_filename}")
plt.close()
...
title = 'Couette Flow - Error Analysis (All Solvers)'
ax.set_title(title, fontsize=TITLE_FONTSIZE, fontweight='bold')
...
plt.tight_layout()
output_filename = './Images/couette_aggregated_error_semilog'
plt.savefig(output_filename + '.png', format='png', dpi=300, bbox_inches='tight')
plt.savefig(output_filename + '.eps', format='eps', bbox_inches='tight')
print(f"Saved: {output_filename}")
plt.close()
...
for solver in riemann_solvers:
    ...
    errors = calculate_errors(solver_data['velocity'], solver_data['y'], 
                              velocity_analytical, y_analytical)
