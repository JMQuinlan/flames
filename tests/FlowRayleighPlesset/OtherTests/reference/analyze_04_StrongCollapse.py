"""
Test 04 - High-pressure-ratio Collapse: compare AMReX R(t) to both RP and
Keller-Miksis. KM should agree better at minimum-radius because the bubble
wall reaches non-trivial Mach numbers.
...
T_END = 6.0e-5   # 60 us covers Chen 2D collapse (~40 us) + rebound
...
def main():
    ...
    print(f"3D Rayleigh classical tau_R = {tau_R*1e6:.3f} us")
    ...
    print(f"KM peak wall Mach              = {M_max_km:.3f}")
    ...
    ax1.legend(loc="best")
    ax1.set_title("Test 04 - Strong Collapse (p_inf/p_g0 = 24.7)")

    # Bottom panel: wall Mach number
    ax2.plot(t_c2d * 1e6, V_c2d / P.c_l, "b-",  lw=2.5, label="Chen 2D wall Mach")
    ax2.plot(t_rp  * 1e6, V_rp  / P.c_l, "0.4", lw=1.0, ls="--", label="3D RP (context)")
    ax2.plot(t_km  * 1e6, V_km  / P.c_l, "0.6", lw=1.0, ls=":",  label="3D KM (context)")
    ax2.axhline(0.0, color="k", lw=0.4, alpha=0.5)
    ax2.axhline(0.1, color="r", lw=0.4, ls=":", alpha=0.5,
                label="M = 0.1 (RP starts to lose validity)")
