...
P = Params(
    rho_l=10.0,        # scaled water analog (matches user's working setup)
    p_inf=500.0,
    mu_l=0.0,
    c_l=63.0,          # informational only (KM not used here)
    gamma_gas=1.4,
    p_g0=750.0,        # 1.5 * p_inf : mild over-pressure, sigma=0
    p_v=0.0,
    sigma=0.0,
    R0=0.02,
    Rdot0=0.0,
    r_inf=0.10,        # half-domain (5*R0)
)
T_END = 1.0e-1   # 100 ms - covers ~0.5 of Chen 2D oscillation period
...
def main():
    ...
    ax.plot(t_km  * 1e3, R_km  * 1e3, "0.6", lw=1.0, ls=":",
    ...
    ax.set_ylabel("R [mm]")
    ax.set_title("Test 01 - Mild Expansion (p_g0/p_inf = 1.5, no surface tension)")
    ...
    print(f"  t at peak = {t_c2d[R_c2d.argmax()] * 1e3:.2f} ms")
    print(f"3D Rayleigh-Plesset (context):")
    print(f"  R_max     = {R_rp.max() * 1e3:.4f} mm   (= {R_rp.max()/P.R0:.3f} * R0)")
    print(f"  t at peak = {t_rp[R_rp.argmax()] * 1e3:.2f} ms")
