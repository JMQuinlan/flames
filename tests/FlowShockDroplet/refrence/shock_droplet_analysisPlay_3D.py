...
# File paths
amrex_output_dir = r'../../../bin/tests/FlowShockDroplet/output_ShockDroplet_3D'
...
output_folder = './ShockDroplet_Analysis_3D'
...
def _write_run_metadata(script_name):
    ...
    with open(meta_path, 'w') as f:
        f.write("# Shock-droplet analysis run metadata\n")
...
_write_run_metadata('shock_droplet_analysis2.py')
...
for i, idx in enumerate(analysis_indices):
    ...
    slc = ds.slice('z', float(ds.domain_center[2]))
    domain_width = X_MAX - X_MIN
    domain_height = Y_MAX - Y_MIN
    resolution = 512
    
    frb = slc.to_frb((domain_width, 'code_length'), resolution, 
                     center=[0.5*(X_MIN+X_MAX), 0.5*(Y_MIN+Y_MAX), float(ds.domain_center[2])],
