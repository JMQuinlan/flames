...
"""
===============================================================================
FlowLaplace single-test analysis  --  3D Young-Laplace SPHERE test (2 sigma/R).
===============================================================================
...
A correct Brackbill CSF holds the bubble at R = R0 (p_gas - p_liquid = 2 sigma/R,
3D sphere, TWO equal curvatures), so R/R0 should stay flat at 1 and the error should stay
...
def find_output_dir(name):
    ...
    candidates = [
        os.path.join("/mmfs1/home/ttryon/flames/bin/tests/FlowLaplace/UNIT_TEST_3D", f"output_{name}"),
        os.path.join(_REPO, "bin", "tests", "FlowLaplace", "UNIT_TEST_3D", f"output_{name}"),
        os.path.join(os.getcwd(), "bin", "tests", "FlowLaplace", "UNIT_TEST_3D", f"output_{name}"),
...
def plot_radius(name, R0, sigma, t, Rx, Ry):
    ...
    ax1.set_title(rf"Laplace test  $R_0={sci_latex(R0)}$,  $\sigma={sci_latex(sigma)}$  "
                  rf"($\Delta p=2\sigma/R_0={sci_latex(2.0 * sigma / R0 if R0 else 0)}$)")
...
def main():
    ...
    print(f"  R0 = {R0:g}, sigma = {sigma:g}, "
          f"2 sigma/R0 = {2.0 * sigma / R0 if R0 else 0:g}, tau = {char_timescale(R0, sigma):.3g}")
