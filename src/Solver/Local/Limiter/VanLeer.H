...
public:
    ...
    VanLeer(IO::ParmParse &pp, std::string name)
    {
        pp_queryclass(name, *this);
    }

    VanLeer(IO::ParmParse &pp)
    {
        pp_queryclass(*this);
    }
    ...
    static void
    Parse(VanLeer &value, IO::ParmParse &pp)
    {
    ...
    /// Van Leer limiter function for two arguments
    Set::Scalar
    vanleer(Set::Scalar a, Set::Scalar b, Set::Scalar small = 1E-12) const
    {
        if (a * b <= 0.0)
            return 0.0; // Different signs or one is zero

        // Van Leer harmonic average: 2*a*b/(a+b)
        return 2.0 * a * b / (a + b + small);
    }

    /// Apply Van Leer limiter to a slope
    Set::Scalar
    apply(Set::Scalar slope_left, Set::Scalar slope_right) const
    {
        return vanleer(slope_left, slope_right);
    }

    /// Apply Van Leer limiter to compute limited slope at cell i
    Set::Scalar
    ...
    /// Reconstruct left and right states at cell interfaces using Van Leer limiter
    void
        ...
        for (int i = 0; i <= n; i++)
        {
            ...
            else
            {
                ...
                for (int k = 0; k < 4; k++)
                {
                    ...
                    W_slopes[k] = vanleer(backward_diff, forward_diff);
                }
    ...
    Test()
    {
        ...
        // Test 1: Basic Van Leer limiter function
        try
        {
            // Same sign, positive
            Set::Scalar result1 = limiter.vanleer(1.0, 2.0);
            if (std::abs(result1 - 4.0 / 3.0) > 1E-10)
            {
                Util::Warning(INFO, "vanleer(1.0, 2.0) should be 4/3, got ", result1);
                Util::Exception(INFO, "Van Leer limiter test failed");
            }

            // Same sign, negative
            Set::Scalar result2 = limiter.vanleer(-1.0, -2.0);
            if (std::abs(result2 - (-4.0 / 3.0)) > 1E-10)
            {
                Util::Warning(INFO, "vanleer(-1.0, -2.0) should be -4/3, got ", result2);
                Util::Exception(INFO, "Van Leer limiter test failed");
            }

            // Different signs
            Set::Scalar result3 = limiter.vanleer(1.0, -2.0);
            if (std::abs(result3) > 1E-10)
            {
                Util::Warning(INFO, "vanleer(1.0, -2.0) should be 0.0, got ", result3);
                Util::Exception(INFO, "Van Leer limiter test failed");
            }

            // One zero
            Set::Scalar result4 = limiter.vanleer(0.0, 2.0);
            if (std::abs(result4) > 1E-10)
            {
                Util::Warning(INFO, "vanleer(0.0, 2.0) should be 0.0, got ", result4);
                Util::Exception(INFO, "Van Leer limiter test failed");
            }

            // Equal values
            Set::Scalar result5 = limiter.vanleer(2.0, 2.0);
            if (std::abs(result5 - 2.0) > 1E-10)
            {
                Util::Warning(INFO, "vanleer(2.0, 2.0) should be 2.0, got ", result5);
                Util::Exception(INFO, "Van Leer limiter test failed");
            }

            Util::Test::SubMessage("Test 1: Basic Van Leer limiter function", 0);
        }
        catch (const std::runtime_error &e)
        {
            failed++;
            Util::Test::SubMessage("Test 1: Basic Van Leer limiter function", 1);
        }
        ...
        try
        {
            ...
            Set::Scalar expected1 = limiter.vanleer(2.0, -1.0);
            if (std::abs(slope1 - expected1) > 1E-10)
            {
                Util::Warning(INFO, "limitSlope at index 2 should be ", expected1, ", got ", slope1);
        ...
        try
        {
            ...
            // Check that slopes are limited correctly
            // For linear data, Van Leer should recover the exact slope
            Set::Scalar expected_slope = 1.0; // Linear increase in density
            Set::Scalar actual_slope = leftStates[1].rho - rightStates[1].rho;
            if (std::abs(actual_slope - expected_slope) > 1E-10)
            {
                Util::Warning(INFO, "For linear data, slope should be ", expected_slope, ", got ", actual_slope);
                Util::Exception(INFO, "Van Leer limiter not recovering exact slope for linear data");
            }
...
#endif // SOLVER_LOCAL_LIMITER_VANLEER_H
